...
Const1_1(scen,T).. Y(T)-X(scen,T)-R(scen,T)*GG -SOLAR(scen,T) =L= Z(scen)*BigM(scen,t) ;

Const1_1_scenario(scen,t)$(ord(t) lt card(t) and (ord(scen) eq counter))..
         Y(T)-X(scen,T)-R(scen,T)*GG -SOLAR(scen,T) =L= 0 ;

Const1_2..   SUM(SCEN, Z(SCEN)) =L= threshold;
